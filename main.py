...
class ML_Control(object):
	def __init__(
			...
			scale = 10, # kernel scaling parameter
	...
	def obtain_trajectories(self):
		...
		x_0 = np.array([self.x_0[0]+ 0.5*np.random.rand()])
	...
	def control(self, x_trajectories, y_trajectories):
		...
		x = Variable((1, self.T_mpc+1)) # opt variables
		u = Variable((1, self.T_mpc)) # opt variables
		...
		for j in range(mpc_sim):	
			...
			x_opt, u_opt = STP_MPC(mus, J_x, J_u, x_max, x_min, init_traj, self.T_mpc, self.x_0, self.x_f)
			x_Jax_temp = x_opt[0]
			u_Jac_temp = u_opt[0]
...
def main():
    ...
    IPython.embed()
    alg.control(x_trajectories, y_trajectories)
